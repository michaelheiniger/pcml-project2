...
def mf_sgd_biased_imp(train, test, num_epochs, gamma, num_features, lambda_b, lambda_r):
    ...
    y = np.random.random((train.shape[1], num_features))
    ...
    #Find non-zero 
    R = np.array([ U.nonzero()[0] for U in train.T ])
    ...
    for it in range(num_epochs):        
        ...
        for n in nz_col:
            y_R_n = y[R[n]]
            sum_Rn = np.sum(y_R_n, axis=0)
            W_n = 1/np.sqrt(len(R[n])) * sum_Rn
            su = 0
            for d in R[n]:
                #W_p = W[:,d] + 1/np.sqrt(len(R[n])) * sum_Rn
                e = train[d,n] - prediction_biased(W_n.T, Z[:,n], mu, user_biases[0, n], item_biases[d,0])
                su += e * Z[:,n]
                user_biases[0, n] += gamma * (e - lambda_b * user_biases[0, n])
                item_biases[d, 0] += gamma * (e - lambda_b * item_biases[d, 0])
                Z[:,n] += gamma * (e *W_n - lambda_r * Z[:,n])
                #W[:,d] += gamma * (e *Z[:,n] - lambda_r * W[:,d])
            start = time.clock()
            y[R[n]] = y_R_n + gamma * (e * 1/np.sqrt(len(R[n])) * Z[:, n] - lambda_r * y_R_n)
            stop = time.clock()
            if i % 10000 == 0:
                print("Computation 2 in ", stop - start, "s")
...
def mf_sgd_biased(train, test, num_epochs, gamma, num_features, lambda_b, lambda_r):
    """ Matrix factorization using GD 
    output:
        rmse_train: for every epoch (stored in array of shape (num_epochs, 1) )
        rmse_test: for every epoch (...)
    ...
    for it in range(num_epochs):        
        ...
        for d, n in nz_train:
            ...
            sum_Rn = np.sum(y_R_n, axis=0)
            W_p = W[:,d] + 1/np.sqrt(len(R[n])) * sum_Rn
            e = train[d,n] - prediction_biased(W_p.T, Z[:,n], mu, user_biases[0, n], item_biases[d,0])
            user_biases[0, n] += gamma * (e - lambda_b * user_biases[0, n])
            item_biases[d, 0] += gamma * (e - lambda_b * item_biases[d, 0])
            Z[:,n] += gamma * (e *W_p - lambda_r * Z[:,n])
            W[:,d] += gamma * (e *Z[:,n] - lambda_r * W[:,d])
            start = time.clock()
            y[R[n]] = y_R_n + gamma * (e * 1/np.sqrt(len(R[n])) * Z[:, n] - lambda_r * y_R_n)
            stop = time.clock()
