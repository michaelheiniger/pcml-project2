...
def get_error(X, W, Z):
    ...
    input:
        X: known ratings
        W: item features matrix
        Z: user features matrix
    output:
...
def init_MF(train, num_features):
    ...
    output:
        user_features: initialized user features matrix
        item_features: initialized item features matrix
    """
...
def update_user_feature(
    ...
    """Update the user features matrix
    input:
        train: the train data
        item_features: the item features matrix
        lambda_user: regularization parameter for the user features matrix
        user_features: the user features matrix
        num_features: the dimension (K) of the subspace
        A_arr: the matrix A (see report)"""
...
def update_item_feature(
    ...
    input:
        train: the train data
        user_features: the user features matrix
        lambda_item: regularization parameter for the item features matrix
        item_features: the item features matrix
        num_features: the dimension (K) of the subspace
        A_arr: the matrix A (see report)"""
    train_c = sp.csr_matrix(train)
...
def cross_validation(ratings, k_indices, k, num_features, lambda_user, lambda_item):
    """ Perform one step of K-Fold Cross-validation for Matrix Factorization with GD 
    input: 
        ratings: the dataset
